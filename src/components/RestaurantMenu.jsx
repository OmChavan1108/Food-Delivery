      ...
      <div className="space-y-4">
        {menu.map((item) => (
          <div key={item.card.info.id}
            className="bg-white p-5 rounded-xl shadow-md border border-gray-200 hover:shadow-lg transition-all flex justify-between items-center" >
            ...
            <button className="px-6 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 hover:scale-105 transition-all" onClick={()=>handlefn(item)}>Add</button>
          </div>
        ))}
      </div>
